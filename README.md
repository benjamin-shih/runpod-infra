...
This repository packages the controller pattern used in prior research work as a standalone Python package named `runpod-research` (import package `runpod_research`) with CLI entry points prefixed by `rpr`. The source repository is currently named `runpod-infra`; the installable package and docs use `runpod-research` for the reusable sweep tooling. It is designed for project repos to import or run without copying controller internals.

## What it provides

- Render one RunPod pod payload per job from a JSON sweep spec.
- Maintain a durable JSON lane queue with restart-safe states, advisory locking, and fsync-backed atomic writes.
- Launch queued lanes only with explicit spend confirmation and configurable launch/concurrency caps.
...
- No live spend without `--confirm-spend`; confirmed controller launches default to one active lane and one new pod per tick unless `--max-concurrent` / `--max-launches-per-tick` are raised after budget approval.
...
- `docs/architecture.md` — package boundaries, lifecycle model, safety posture, and extension points.
- `docs/api.md` — stable Python import surface for downstream tooling.
